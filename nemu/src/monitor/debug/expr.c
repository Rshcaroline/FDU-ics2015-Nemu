...
enum {
	NOTYPE = 256, EQ , NEQ , AND , OR , NUMBER , HNUMBER , MINUS , POINTOR , REGISTER , MARK
...
static struct rule {
	...
	{"\\b[a-zA-Z_0-9]+" , MARK , 0},		// mark if it is a variant
...
#define NR_REGEX (sizeof(rules) / sizeof(rules[0]) )  //how many rules
	...
	while(e[position] != '\0') {
		...
		for(i = 0; i < NR_REGEX; i ++) {
			if(regexec(&re[i], e + position, 1, &pmatch, 0) == 0 && pmatch.rm_so == 0) {
				...
				char *tmp = e + position + 1;  //contents after $
				...
				switch(rules[i].token_type) {
					...
					case REGISTER:
						...
						//copy the contents after $ ie the name of the register
...
uint32_t eval(int l,int r) {
	if (l > r){Assert (l>r,"What's happening!\n");return 0;}
	if (l == r) {
	uint32_t num = 0;
	if (token[l].type == NUMBER)  //10
		sscanf(token[l].str,"%d",&num);
	if (token[l].type == HNUMBER)  //16
